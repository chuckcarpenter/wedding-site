  ...
  var Gallery = function () {
    ...
    function cleanSlideMeta() {
      var slide = els.gallery.one(".slide.sqs-active-slide"),
        ...
        slideMeta = els.gallery.one(
          '.slide-meta[data-slide-id="' + activeSlideId + '"]'
        );
      ...
      if (slideMeta) {
        if (els.body.hasClass(classes.crop)) {
          ...
          if (els.body.hasClass(classes.circlesNav)) {
            slideMeta.setStyle(
              "top",
              (parseFloat(slide._node.clientHeight) -
                parseFloat(slide.one("img")._node.clientHeight)) /
                2
            );
    ...
    function hasSlideMeta(slide, slideMeta, check) {
      ...
      if (slideMeta) {
        ...
        if (
          (title && title._node.innerHTML) ||
          (description && description._node.innerHTML)
        ) {
          if (check) {
            found = true;
          } else {
    ...
    function slideChange(e) {
      var activeSlide = els.gallery.one(".slide.sqs-active-slide"),
        ...
        activeSlideId =
          (activeSlide && activeSlide.getAttribute("data-slide-id")) || null,
        bottom;
      ...
      if (activeSlideId) {
        hasSlideMeta(
          activeSlide,
          els.gallery.one('.slide-meta[data-slide-id="' + activeSlideId + '"]')
        );
        ...
        if (activeImage && els.body.hasClass(classes.circlesNav)) {
          var setBottom = function () {
            bottom = Math.max(
              0,
              (parseFloat(activeSlide._node.clientHeight) -
                parseFloat(activeImage._node.clientHeight)) /
                2
            );
    ...
    function buildGallery() {
      var galleryLoop = getTweakValue("gallery-loop"),
        ...
        galleryAutoplaySpeed =
          getTweakValue("galleryAutoplaySpeed", true) * 1000,
      ...
      slideshowAspectRatio =
        parseInt(galleryHeight.split(":")[1], 10) /
        parseInt(galleryHeight.split(":")[0], 10);
      ...
      els.gallery
        .one(".slides-controls")
        .setStyle("height", isGalleryGrid ? null : galleryHeight);
      els.slideWrapper.setStyle(
        "minHeight",
        isGalleryGrid ? null : galleryHeight
      );
      ...
      galleryObj = new Y.Squarespace.Gallery2({
        ...
        loaderOptions: {
          mode: isGalleryGrid
            ? getTweakValue("aspect-ratio") === "auto"
              ? "none"
              : "fill"
            : galleryAutoCrop
            ? "fill"
    ...
    function interaction(e, leave) {
      ...
      if (
        e._event.target &&
        (e._event.target.localName === "img" ||
          e._event.target.className === "sqs-video-opaque" ||
    ...
    function isVideo() {
      ...
      return (
        activeSlide &&
        activeSlide.getAttribute("data-type") === "video" &&
        activeSlide.one("iframe")
      );
    }
    ...
    function tweaks() {
      ...
      watchTweak("gallery-navigation", function (newTweakValue) {
        if (
          newTweakValue === "Thumbnails" &&
          !els.gallery.one(".thumbnail-wrapper img[src]")
        ) {
          buildGallery();
        }
